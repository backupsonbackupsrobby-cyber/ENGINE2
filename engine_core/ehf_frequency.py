...
class EHFFrequencyEngine:
    ...
    def get_cognitive_state(
        self, circadian_phase: CircadianPhase = None
    ) -> CognitiveState:
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if m.heart_rate > 100:
            recommendations.append(
                "🫀 High HR: Take deep breathing breaks (4-7-8 technique)"
            )
        ...
        if m.sleep_quality < 70:
            recommendations.append(
                "😴 Poor sleep: Avoid screens 1hr before bed, cool room (16-18°C)"
            )

        # Stress recommendations
        if m.stress_score > 60:
            recommendations.append(
                "🧘 High stress: 5-min meditation or cold exposure (15s)"
            )

        # Energy recommendations
        if m.energy_level < 50:
            recommendations.append(
                "⚡ Low energy: Hydrate (500ml water), take 10-min walk"
            )

        # Glucose recommendations
        if m.glucose_level > 130:
            recommendations.append(
                "🍎 High glucose: Light activity after meals, vinegar before eating"
            )

        # Circadian recommendations
        if circadian == CircadianPhase.POST_LUNCH_DIP:
            recommendations.append(
                "☀️ Afternoon dip: 10-min sunlight exposure, short walk"
            )
        ...
        if cognitive == CognitiveState.PEAK_FOCUS:
            recommendations.append(
                "🎯 Peak focus time: Start deep work now (90-min blocks)"
            )
        elif cognitive == CognitiveState.RECOVERY:
            recommendations.append(
                "🔄 Recovery phase: Switch to admin tasks, light meetings"
            )

        return (
            recommendations
            if recommendations
            else ["✅ Optimal conditions - maintain current state"]
        )
