...
class TRONRhythmEngine:
    ...
    async def heartbeat_phase(self):
        ...
        print(f"  💓 Heartbeat: Devices={heartbeat['devices']}, Energy={heartbeat['energy']:.1f}%")
    ...
    async def execution_phase(self, actions: List[Dict]):
        ...
        for action in actions:
            print(f"     • {action.get('device', 'unknown')}: {action.get('command', 'unknown')}")
    ...
    async def run_tron_cycle(self, zha_state: Dict = None, actions: List[Dict] = None):
        ...
        accuracy = max(0, 100 - abs((cycle_duration - target_duration) / target_duration * 100))
        ...
        while True:
            try:
                ...
                if len(self.zha_devices) > 0:
                    actions = [
                        {
                            'device': list(self.zha_devices)[0] if self.zha_devices else 'none',
                            'command': 'sync',
                        }
                    ]
