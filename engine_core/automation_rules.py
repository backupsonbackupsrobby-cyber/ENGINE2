...
class AutomationRulesEngine:
    ...
    def create_rule(self, rule_id: str, name: str, description: str = "") -> AutomationRule:
    ...
    async def execute_rules(self, rule_ids: List[str], 
                           sync_cycle: int = None) -> Dict:
...
async def setup_example_automations(engine: AutomationRulesEngine,
                                   zha: 'ZHAIntegration'):
    ...
    morning = engine.create_rule(
        "morning_routine",
        "Morning Routine",
        "Lights on, thermostat warm, coffee ready"
    )
    morning.add_trigger(AutomationTrigger(
        trigger_type=TriggerType.TIME,
        entity_id="alarm_bedroom",
        value="07:00:00",
        description="Trigger at 7 AM"
    ))
    morning.add_action(AutomationAction(
    ...
    night_mode = engine.create_rule(
        "night_mode",
        "Night Mode",
        "Lights on at sunset, security mode active"
    )
    night_mode.add_trigger(AutomationTrigger(
        trigger_type=TriggerType.SUN,
        entity_id="sun",
        value="sunset"
    ))
    night_mode.add_condition({
        'entity': 'lock_front_door',
        'condition': 'equals',
        'value': 'locked'
    })
    night_mode.add_action(AutomationAction(
        device_id="light_living_room",
        service="turn_on"
    ))
    night_mode.add_action(AutomationAction(
    ...
    temp_control.add_trigger(AutomationTrigger(
    ...
    temp_control.add_action(AutomationAction(
        device_id="thermostat_bedroom",
        service="set_temperature",
        data={'temperature': 22}
    ))
