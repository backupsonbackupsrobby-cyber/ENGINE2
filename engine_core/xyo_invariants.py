...
class BoundWitness:
    ...
    def _compute_witness_hash(self) -> str:
        ...
        combined = (
            self.location_inv.hash + 
            self.time_inv.hash + 
            self.identity_inv.hash
        )
...
class SymPyLocationSync:
    ...
    def solve_container_health(
        ...
        try:
            ...
            health_value = container_health.subs([(x, location_x), (y, location_y), (t, time_offset)])
...
class ENGINEXYOSync:
    ...
    def register_container(
        ...
        health_data = SymPyLocationSync.solve_container_health(
            location_x,
            location_y,
            time_offset / 3600.0  # Convert to hours
        )
    ...
    def generate_sync_manifest(self) -> Dict:
        ...
        for bw in self.bound_witnesses:
            ...
            try:
                health_scores.append(float(bw.to_dict()["bound_witness"].get("health", 0.5)))
            except:
...
def main():
    ...
    sync = ENGINEXYOSync(
        engine_location="San Francisco, CA",
        device_id="ENGINE-NODE-001"
    )
    ...
    for container_name, container_id, lat, lon in containers:
        ...
        print(f"  Witness Hash: {result['xyo_bound_witness']['bound_witness']['location']['hash'][:16]}...")
    ...
    print()
    print("╔════════════════════════════════════════════════════════════╗")
    print(f"║  Overall Health Score: {manifest['total_health_score']:.2%}                        ║")
