...
class UnifiedZHAIntegration:
    ...
    async def discover_all_devices(self) -> List[UniversalSmartDevice]:
        ...
        for device in discovered_devices:
            ...
            print(
                f"  ✓ {device.name} [{proto}] {f'({device.name_cn})' if device.name_cn else ''}"
            )
    ...
    async def set_device_state(
        ...
        display_name = (
            f"{device.name_cn} ({device.name})" if device.name_cn else device.name
        )
    ...
    def create_device_group(
        self, group_name: str, group_name_cn: str, device_ids: List[str]
    ) -> bool:
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        zha.create_device_group(
            "living_room", "客厅", ["hue_light_1", "tuya_light_1", "tuya_plug_1"]
        )

        zha.create_device_group("bedroom", "卧室", ["innr_light_1", "aqara_ac_1"])
