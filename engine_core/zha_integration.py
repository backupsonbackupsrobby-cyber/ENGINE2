...
class ZHAIntegration:
    ...
    async def set_device_state(self, device_id: str, new_state: ZHAState, 
    ...
    async def control_device_group(self, group_name: str, command: str, 
                                   value: any = None) -> Dict:
        ...
        for device_id in device_ids:
            if device_id in self.devices:
                ...
                if command == 'toggle':
                    new_state = ZHAState.OFF if device.state == ZHAState.ON else ZHAState.ON
                else:
    ...
    async def create_automation(self, rule_name: str, trigger: Dict, actions: List[Dict]) -> bool:
        ...
        for rule_name, rule in self.automation_rules.items():
            ...
            if trigger.get('device_id') == trigger_data.get('device_id'):
                if trigger.get('state') == trigger_data.get('state'):
                    ...
                    print(f"[ZHA] Automation triggered: {rule_name} ({rule['execution_count']} times)")
    ...
    async def sync_with_tron(self, tron_cycle: int) -> Dict:
        ...
        print(f"[ZHA-TRON] Syncing {len(pending_changes)} changes with cycle {tron_cycle}")
...
if __name__ == "__main__":
    async def demo():
        ...
        await zha.set_device_state('thermostat_bedroom', ZHAState.IDLE, 
                                   {'temperature': 22.0})
        ...
        print(f"  Commands: {status['total_commands']} ({status['success_rate']:.1f}% success)")
