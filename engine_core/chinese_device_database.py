...
class MiHomeIntegration:
    ...
    async def set_property(self, device_id: str, property_name: str, value: any) -> bool:
        """Set device property"""
...
class ChineseSmartDeviceDatabase:
    ...
    def get_device_info(device_model: str) -> Dict:
        ...
        return ChineseSmartDeviceDatabase.DEVICE_DATABASE.get(
            device_model,
            {'error': f'Model {device_model} not found'}
        )
    ...
    async def execute_unified_scene(self, scene_name: str) -> Dict:
        ...
        scenes = {
            'morning': [  # 早晨场景
                ...
                {'platform': 'mi_home', 'device': 'mi_ac_001', 'action': 'set_temp', 'value': 25},
            ],
            'sleep': [  # 睡眠场景
                {'platform': 'tuya', 'device': 'tuya_light_001', 'action': 'turn_off'},
                {'platform': 'aqara', 'device': 'aqara_light_001', 'action': 'turn_off'},
            ],
            'away': [  # 离家场景
                {'platform': 'tuya', 'devices': 'all', 'action': 'turn_off'},
                {'platform': 'aqara', 'device': 'aqara_sensor_001', 'action': 'start_monitoring'},
            ],
        }
